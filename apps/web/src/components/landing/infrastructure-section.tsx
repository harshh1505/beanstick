...
const techStack = [
  { name: "0G Chain", desc: "Escrow & orderbook" },
  { name: "0G Compute", desc: "zkTLS verification" },
  { name: "0G Storage", desc: "Proof persistence" },
  { name: "Gensyn AXL", desc: "P2P agent mesh" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                Built on 0G's modular AI blockchain. Escrow, compute verification, and storage—all onchain, all verifiable.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                ...
                <span className="text-2xl text-muted-foreground">layers</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                Chain for escrow, Compute for proofs, Storage for evidence, AXL for P2P messaging.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">x402</span>
              <span className="block text-sm text-muted-foreground mt-2">HTTP-native payments</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">MCP</span>
              <span className="block text-sm text-muted-foreground mt-2">Semantic tool calls</span>
            </div>
          </div>
        </div>
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <span className={`w-2 h-2 rounded-full transition-colors ${
                  activeTech === index ? "bg-emerald-400" : "bg-foreground/20"
                }`} />
                <span className="text-xs font-mono text-muted-foreground uppercase tracking-wider">
                  active
                </span>
              </div>
              <span className="font-medium block mb-1">{tech.name}</span>
